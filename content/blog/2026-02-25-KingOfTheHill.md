...
subtitle: Building Challenges For This Week's Club Event
date: 2026-02-25
# lastmod: 2026-02-03
...
# The CTF Itself #
The point system of this CTF is pretty straightforward. Students are on some teams and competing for flags. Instead of just getting points for capturing the flag, the points would go up for how long you have the flag before other teams capture it. Once other teams start getting the flag from the same challenge, the points accumulated would be split across the teams until every team has gotten the flag. At this point, it would no longer generate points. 

Since our club meetings are only an hour long each week, I will need to keep the difficulty in the easy to medium level range.

The point breakdown for a group of 4 teams would look like this:
| Who Has Flag | Team 1 | Team 2 | Team 3 | Team 4 |
| :------------: | :------: | :-----: | :------: | :------: |
| Only 1 | 100% / min | 0% / min | 0% / min | 0% / min |
...
What makes creating these challenges hard is that I need to balance these things for any sort of hardware that the competitors are using. I built my pc for password cracking, but I need to take into account the people using a raspberry pi. The goal of making these challenges is to make passwords complex enough to not be brute forced in the time frame, but fast enough that low end computers could do it within that same time.

My process for creating these types of challenges goes like this:
...
Once I had an idea of the type of challenges and a story for each challenge, I generated hashes. Typically I work with unsalted MD5 or NTLM hashes, but that can get predicatable so I changed it up for the medium challenge.
...
Since I've never really made any network challenges before this one. I wasn't sure where to start. In previous CTFs, I was almost always given a pcap file and asked questions about it. I figured the best way to go about this would be to start a packet capture in my test lab, and then send a file from one device to another. 
